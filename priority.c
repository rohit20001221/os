...
void sort_arr2(int arr1[], int arr2[], int arr3[], int n){
    for(int i=0; i < n; i++){
        for(int j = i; j < n; j++){
            if(arr1[i] > arr1[j]){
                swap(&arr1[i], &arr1[j]);
...
void findavgTime(int processes[], int priority[], int n, int bt[]){
    int wt[n], tat[n], total_wt=0, total_tat=0;

    sort_arr2(priority, bt, processes, n);
...
int main(){
    ...
    findavgTime(processes, priority, n, burst_time);
    return 0;
}